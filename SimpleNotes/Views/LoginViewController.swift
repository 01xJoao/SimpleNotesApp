...
public class LoginViewController : BaseViewController<LoginViewModel> {
    override public func viewDidLoad() {
        super.viewDidLoad()
        self.view.backgroundColor = AppColors.red
        
        let button = UIButton(frame: CGRect(x: 0, y: 50, width: 200, height: 50))
        button.backgroundColor = AppColors.mainBlue
        ...
        button2.backgroundColor = AppColors.green
    ...
    @objc func _navigateAccount(sender: UIButton){
        viewModel.navigateToCreateAccountCommand()
    }
}
